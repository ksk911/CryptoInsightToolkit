...
with col1:
    if hedge_result:
        st.metric(
            "Hedge Ratio (β)", 
            f"{hedge_result['hedge_ratio']:.4f}",
            help="Optimal ratio from OLS regression"
        )
    else:
...
with col2:
    if hedge_result:
        st.metric(
            "R² (Fit Quality)", 
            f"{hedge_result['r_squared']:.4f}",
            help="How well the prices are correlated"
        )
    else:
        st.metric("R²", "N/A")
...
with col3:
    if 'zscore' in current_metrics:
        st.metric(
            "Current Z-Score", 
            f"{current_metrics['zscore']:.2f}",
            help="Standardized spread value"
        )
    else:
...
with col4:
    if adf_result:
        st.metric(
            ...
            help="ADF test result for mean reversion"
        )
    else:
...
# ============================================
# CHART 2 & 3: SPREAD & Z-SCORE ANALYSIS (SEPARATED)
# ============================================
...
if spread is not None and zscore is not None and not zscore.dropna().empty:
    ...
    with col1:
        alert_components.render_alert_button(
            metric='spread',
            default_threshold=650.0,
            step=10.0,
            key_suffix='spread_chart'
        )
    
    with col2:
        # Delete all spread alerts button
    ...
    fig_zscore.add_hline(
        ...
        line_dash="dash", 
        line_color="red", 
        annotation_text=f"SHORT Entry (+{entry_threshold})"
    )
    ...
    fig_zscore.add_hline(
        ...
        line_dash="dash", 
        line_color="green", 
        annotation_text=f"LONG Entry (-{entry_threshold})"
    )
    ...
    with col1:
        alert_components.render_alert_button(
            ...
            step=0.1,
            key_suffix='zscore_chart'
        )
    
    with col2:
        # Delete all z-score alerts button
...
st.markdown("---")
st.markdown("")  # Extra spacing before next section
...
if correlation is not None and not correlation.dropna().empty:
    ...
    with col1:
        alert_components.render_alert_button(
            ...
            step=0.1,
            key_suffix='correlation_chart'
        )
    
    with col2:
        # Delete all correlation alerts button
...
with col1:
    ...
    if hedge_result:
        ...
        st.json({
            ...
            "Model Fit": fit_quality
        })
        
        if hedge_result['r_squared'] < 0.5:
            st.warning("⚠️ Low R²: Prices may not be well-correlated")
    else:
...
with col2:
    ...
    if adf_result:
        ...
        if not adf_result['is_stationary']:
            st.warning("⚠️ Non-stationary spread: Not ideal for mean reversion trading")
    else:
